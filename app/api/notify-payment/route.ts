import { NextResponse } from "next/server";
import { Resend } from "resend";
import { formatNaira } from "@/lib/products";
import { ADMIN_EMAILS, BUSINESS_BANK_DETAILS, WHATSAPP_DISPLAY_NUMBER } from "@/lib/checkout";
import {
  buildOrderItemRowsHtml,
  getEmailFromAddress,
  getEmailReplyToAddress,
  type EmailOrderLine,
} from "@/lib/email";

type NotifyPaymentRequestBody = {
  orderRef: string;
  customerName: string;
  customerEmail: string;
  customerPhone: string;
  deliveryZoneName: string;
  deliveryFee: number;
  lines: EmailOrderLine[];
  subtotal: number;
  total: number;
};

function buildCustomerHtml(order: NotifyPaymentRequestBody): string {
  return `
    <div style="font-family:sans-serif;color:#2C2E1E;max-width:480px;margin:0 auto;">
      <h1 style="font-size:20px;">Macho Meats — Order ${order.orderRef}</h1>
      <p>Hi ${order.customerName}, thanks — we've noted your payment of ${formatNaira(order.total)} and it's now awaiting confirmation.</p>
      <p>We'll be in touch on WhatsApp once it's verified to get your order moving. If you haven't already, you can send your payment screenshot to ${WHATSAPP_DISPLAY_NUMBER} to speed that up.</p>
    </div>
  `;
}

function buildAdminHtml(order: NotifyPaymentRequestBody): string {
  const rows = buildOrderItemRowsHtml(order.lines);

  return `
    <div style="font-family:sans-serif;color:#2C2E1E;max-width:480px;margin:0 auto;">
      <h1 style="font-size:20px;">Payment claimed — Order ${order.orderRef}</h1>
      <p>${order.customerName} says they've paid. Check ${BUSINESS_BANK_DETAILS.accountNumber} (${BUSINESS_BANK_DETAILS.bankName}) for a transfer matching the total below, then confirm with the customer.</p>
      <table style="width:100%;border-collapse:collapse;margin-top:16px;">
        ${rows}
        <tr><td style="padding:8px 0;">Delivery — ${order.deliveryZoneName}</td><td style="padding:8px 0;text-align:right;">${formatNaira(order.deliveryFee)}</td></tr>
        <tr><td style="padding:12px 0;font-weight:bold;border-top:1px solid #ddd;">Total</td><td style="padding:12px 0;text-align:right;font-weight:bold;border-top:1px solid #ddd;">${formatNaira(order.total)}</td></tr>
      </table>
      <p style="margin-top:16px;">Customer email: ${order.customerEmail}<br/>Customer WhatsApp: ${order.customerPhone}</p>
    </div>
  `;
}

export async function POST(request: Request) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) {
    return NextResponse.json(
      { error: "Email sending isn't configured yet (missing RESEND_API_KEY)." },
      { status: 501 }
    );
  }

  const order = (await request.json()) as NotifyPaymentRequestBody;

  if (!order.customerEmail || !order.orderRef || !order.lines?.length) {
    return NextResponse.json({ error: "Missing order details." }, { status: 400 });
  }

  const resend = new Resend(apiKey);
  const from = getEmailFromAddress();
  const replyTo = getEmailReplyToAddress();

  const [customerResult, adminResult] = await Promise.all([
    resend.emails.send({
      from,
      replyTo,
      to: order.customerEmail,
      subject: `Payment received — Order ${order.orderRef}`,
      html: buildCustomerHtml(order),
    }),
    resend.emails.send({
      from,
      replyTo,
      to: ADMIN_EMAILS,
      subject: `Payment claimed — Order ${order.orderRef} — ${formatNaira(order.total)}`,
      html: buildAdminHtml(order),
    }),
  ]);

  if (customerResult.error || adminResult.error) {
    return NextResponse.json(
      { error: customerResult.error?.message ?? adminResult.error?.message },
      { status: 502 }
    );
  }

  return NextResponse.json({ ok: true });
}
